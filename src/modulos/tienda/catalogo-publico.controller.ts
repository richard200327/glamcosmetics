...
export class CatalogoPublicoController {
  ...
  public obtenerProductos(
    ...
    @Query('precioMinimo') precioMinimo?: string,
    ...
    return this.catalogoPublicoService.obtenerProductos({
      ...
      precioMinimo: precioMinimo !== undefined && precioMinimo !== '' && Number(precioMinimo) >= 0 ? Number(precioMinimo) : undefined,
      precioMaximo: precioMaximo !== undefined && precioMaximo !== '' && Number(precioMaximo) >= 0 ? Number(precioMaximo) : undefined,
      pagina: Math.max(Number(pagina) || 1, 1),
      limite: destacados === 'true' ? Math.min(Math.max(Number(limite) || 5, 1), 5) : Math.min(Math.max(Number(limite) || 24, 1), 60)
    });
  }

  @Get('sugerencias')
  public obtenerSugerencias(@Query('texto') texto?: string) {
    return this.catalogoPublicoService.obtenerSugerencias((texto ?? '').slice(0, 80));
  }
